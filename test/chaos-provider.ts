...
const CHAOS_CALLS: { id: string; name: string; arguments: unknown; expect: string }[] = [
	{
		id: "chaos-read-alias",
		name: "read",
	...
	},
	{
		id: "chaos-edit-flat",
		name: "edit",
		arguments: { file_path: "fixture-b.txt", old_string: "alpha", new_string: "omega" },
		expect: "renameAliasedField + foldFlatEditFields",
	},
	{
		id: "chaos-edit-nested",
		name: "edit",
		arguments: { path: "fixture-c.txt", edits: [{ old_string: "gamma", new_string: "theta" }] },
	...
	},
	{
		...
		expect: "unrepairable -> retry error (content null has no alias to recover from)",
	},
];
...
function chaosStream(model: Model<any>, context: Context): AssistantMessageEventStream {
		...
		if (assistantTurns === 0) {
			...
			for (const message of context.messages) {
				...
				const text = message.content
					...
					.join("\n");
				for (const match of text.matchAll(/<repair_note>([\s\S]*?)<\/repair_note>/g)) {
