...
describe("markdown auto-links", () => {
  test("unwraps only the degenerate case", () => {
    expect(unwrapMarkdownAutoLinks("/home/x/[notes.md](http://notes.md)")).toBe(
      "/home/x/notes.md",
    );
    expect(unwrapMarkdownAutoLinks("[click](https://x.com)")).toBe(
      "[click](https://x.com)",
    );
  });
...
describe("unrepairable input", () => {
  ...
  test("fingerprint is stable for the same failure shape", () => {
    const a = repairToolInput({
      toolName: "read",
      schema: readSchema,
      input: { x: 1 },
      config: REPAIR_CONFIGS.read,
    });
    const b = repairToolInput({
      toolName: "read",
      schema: readSchema,
      input: { x: 2 },
      config: REPAIR_CONFIGS.read,
