...
type RegisteredTool = {
	...
	execute: (id: string, params: unknown, signal?: AbortSignal, onUpdate?: unknown, ctx?: unknown) => Promise<any>;
};

const tools = new Map<string, RegisteredTool>();
const eventHandlers = new Map<string, (event: unknown, ctx: unknown) => Promise<void>>();
...
async function runLikePi(toolName: string, rawArgs: unknown, toolCallId = "call-1") {
	...
	return tool.execute(toolCallId, validated, undefined, undefined, { cwd: workDir });
}
...
beforeAll(async () => {
	...
	if (!sessionStart) throw new Error("extension did not subscribe to session_start");
...
describe("read", () => {
	...
	test("markdown auto-linked path is unwrapped", async () => {
		...
		const result = await runLikePi("read", { path: `${workDir}/[notes.md](http://notes.md)` });
		const text = resultText(result);
...
describe("edit", () => {
	test("claude-code style old_string/new_string edits the file", async () => {
		...
		const result = await runLikePi("edit", { file_path: file, old_string: "beta", new_string: "delta" });
...
describe("unrepairable input", () => {
	test("raises a model-readable retry error before pi's Convert can corrupt it", async () => {
		await expect(runLikePi("read", { nothing: "useful" })).rejects.toThrow('Invalid input for tool "read"');
	});

	test("write with null content errors instead of writing the string 'null'", async () => {
		await expect(runLikePi("write", { path: join(workDir, "never.txt"), content: null })).rejects.toThrow(
...
describe("TUI indicator", () => {
	...
	test("repaired calls get the 🔨 indicator appended to their rendered result", async () => {
		...
		const result = await runLikePi("read", { file_path: file }, "call-indicator");
		const tool = tools.get("read") as any;
		const component = tool.renderResult(result, { expanded: false }, fakeTheme, {
