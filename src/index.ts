...
import { repairToolInput, type RepairResult } from "./repair-engine.ts";
import { loadDisplaySettings, saveDisplaySettings, type RepairDisplaySettings } from "./settings.ts";
...
export default function toolRepairExtension(pi: ExtensionAPI) {
	...
	const stashRepair = (tool: string, argsJson: string, rules: string[], notes: string[]) => {
	...
	const diag = (tool: string, result: RepairResult) => {
		...
		const rules = result.rulesFired.length > 0 ? result.rulesFired.join(",") : "none";
	...
	const indicatorLines = (info: RepairInfo, theme: { fg?: (color: string, text: string) => string }): string[] => {
		...
		for (const [name, factory] of Object.entries(BUILTIN_FACTORIES)) {
			...
			pi.registerTool({
				...
				prepareArguments(raw: unknown) {
					...
					const result = repairToolInput({ toolName: name, schema: original.parameters, input: shimmed, config });
					...
					if (result.outcome === "repaired") {
						stashRepair(name, JSON.stringify(result.args), result.rulesFired, result.notes);
						return result.args;
					}
				...
				async execute(toolCallId, params, signal, onUpdate, ctx) {
					...
					if (repair) {
						repairInfoByCallId.set(toolCallId, { rules: repair.rules, notes: repair.notes });
						try {
							pi.appendEntry(REPAIR_ENTRY_TYPE, { toolCallId, tool: name, rules: repair.rules, notes: repair.notes });
						} catch {
					...
					if (repair && repair.notes.length > 0) {
						const noteText = repair.notes.map((note) => `<repair_note>${note}</repair_note>`).join("\n");
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			for (const entry of ctx.sessionManager.getEntries()) {
				...
				if (e.type === "custom" && e.customType === REPAIR_ENTRY_TYPE && typeof e.data?.toolCallId === "string") {
	...
	pi.on("model_select", async (event, ctx) => {
		currentModelId = (event as { model?: { id?: string } }).model?.id ?? ctx.model?.id ?? currentModelId;
	});

	pi.registerCommand("repair-settings", {
		description: "Toggle the tool-repair indicator (🔨) and repair-note display",
		handler: async (_args, ctx) => {
			for (;;) {
				...
				const choice = await ctx.ui.select("Tool repair display settings", [indicatorLabel, notesLabel, "Close"]);
				if (choice === undefined || choice === "Close") break;
				if (choice === indicatorLabel) displaySettings.showIndicator = !displaySettings.showIndicator;
				if (choice === notesLabel) displaySettings.showNotes = !displaySettings.showNotes;
	...
	pi.registerCommand("repair-stats", {
		...
		handler: async (_args, ctx) => {
			...
			const byTool = new Map<string, { repaired: number; unrepairable: number }>();
			const byRule = new Map<string, number>();
			const byModel = new Map<string, { repaired: number; unrepairable: number }>();
			for (const record of records) {
				const tool = byTool.get(record.tool) ?? { repaired: 0, unrepairable: 0 };
				tool[record.outcome] += 1;
				byTool.set(record.tool, tool);
				const model = byModel.get(record.model ?? "unknown") ?? { repaired: 0, unrepairable: 0 };
			...
			const lines = [`Tool repair telemetry (${records.length} events)`, "", "By tool:"];
			for (const [tool, counts] of [...byTool].sort((a, b) => b[1].repaired - a[1].repaired)) {
				lines.push(`  ${tool}: ${counts.repaired} repaired, ${counts.unrepairable} unrepairable`);
			}
			lines.push("", "By model:");
			for (const [model, counts] of [...byModel].sort((a, b) => b[1].repaired - a[1].repaired)) {
				lines.push(`  ${model}: ${counts.repaired} repaired, ${counts.unrepairable} unrepairable`);
			}
