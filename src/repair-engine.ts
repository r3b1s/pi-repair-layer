...
function collectErrors(schema: TSchema, value: unknown): RawIssue[] {
	...
	for (const error of Value.Errors(schema, value)) {
		out.push({
			keyword: String((error as { keyword?: unknown }).keyword ?? ""),
			instancePath: String((error as { instancePath?: unknown }).instancePath ?? ""),
	...
	for (const issue of collectErrors(schema, value)) {
		if (issue.keyword === "required") {
			...
			for (const key of missing) {
				...
				push(
					{ parent: container, key, keyword: issue.keyword, expected: undefined },
		...
		const key = segments[segments.length - 1] as string | number;
...
const renameAliasedField: IssueRule = (site, toolName, config) => {
	...
	for (const alias of aliases) {
		...
		parent[key] = value;
		delete parent[alias];
		return `Renamed \`${alias}\` to \`${key}\` for tool "${toolName}". \`${alias}\` is not a valid field for this tool — use \`${key}\` next time.`;
	}
	return false;
};
...
const dropNullOrUndefinedField: IssueRule = (site, toolName) => {
	...
	return `Dropped ${kind} \`${key}\` from tool "${toolName}". Omit optional fields entirely rather than sending ${kind}.`;
};
...
const wrapBareStringAsArray: IssueRule = (site, toolName) => {
	...
	return `Wrapped your bare string in a single-element array for \`${String(key)}\` in tool "${toolName}". Send an array (e.g. \`["foo"]\`) next time.`;
};
...
function formatRetryMessage(toolName: string, issues: RawIssue[], input: unknown): string {
	const lines = issues
		.slice(0, 8)
		.map((issue) => `  • ${issue.instancePath === "" ? "(root)" : issue.instancePath}: ${issue.message ?? issue.keyword}`);
	let received: string;
	try {
...
export function repairToolInput(options: {
	...
	if (schemaAccepts(schema, current)) {
		if (rulesFired.length === 0) {
			return { outcome: "valid", args: input, rulesFired, notes, issueSummary: undefined, fingerprint: undefined, retryMessage: undefined };
		}
		return { outcome: "repaired", args: current, rulesFired, notes, issueSummary: undefined, fingerprint: undefined, retryMessage: undefined };
	}
	...
	if (typeof current === "string") {
		...
		const parsed = trimmed.startsWith("{") && trimmed.endsWith("}") ? tryParseJson(trimmed) : undefined;
		if (isPlainObject(parsed)) {
			...
			fire(
				...
				`Wrapped your bare string as \`{${field}: ${wrapInArray ? '["..."]' : '"..."'}}\` for tool "${toolName}". Call this tool with a JSON object next time, not a bare string.`,
			);
		}
		if (isPlainObject(current)) unwrapPathFields(current);
	}
	...
	if (schemaAccepts(schema, probe)) {
		...
		return { outcome: "repaired", args: probe, rulesFired, notes, issueSummary, fingerprint, retryMessage: undefined };
	}
	return unrepairable();
}
